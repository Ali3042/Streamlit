coords = {
    'AF': [34.5333, 69.1667], # Afghanistan
    'AX': [60.1167, 19.9000], # Åland Islands
    'AL': [41.3275, 19.8187], # Albania
    'DZ': [36.7528, 3.0420],  # Algeria
    'AS': [-14.3064, -170.6950], # American Samoa
    'AD': [42.5063, 1.5218],   # Andorra
    'AO': [-8.8390, 13.2894],  # Angola
    'AI': [18.2206, -63.0686], # Anguilla
    'AQ': [-75.2500, 0.0000],  # Antarctica
    'AG': [17.0608, -61.7964], # Antigua and Barbuda
    'AR': [-34.6037, -58.3816],# Argentina
    'AM': [40.1772, 44.5035],  # Armenia
    'AW': [12.5211, -69.9683], # Aruba
    'AU': [-25.2744, 133.7751],# Australia
    'AT': [48.2082, 16.3738],  # Austria
    'AZ': [40.4093, 49.8671],  # Azerbaijan
    'BS': [25.0343, -77.3963], # Bahamas
    'BH': [26.0667, 50.5577],  # Bahrain
    'BD': [23.8103, 90.4125],  # Bangladesh
    'BB': [13.1939, -59.5432], # Barbados
    'BY': [53.9006, 27.5590],  # Belarus
    'BE': [50.8503, 4.3517],   # Belgium
    'BZ': [17.1899, -88.4976], # Belize
    'BJ': [6.3703, 2.3912],    # Benin
    'BM': [32.3078, -64.7505], # Bermuda
    'BT': [27.5142, 89.6362],  # Bhutan
    'BO': [-16.4897, -68.1193],# Bolivia
    'BQ': [12.1784, -68.2385], # Bonaire, Sint Eustatius and Saba
    'BA': [43.8563, 18.4131],  # Bosnia and Herzegovina
    'BW': [-24.6282, 25.9231], # Botswana
    'BV': [-54.4208, 3.3464],  # Bouvet Island
    'BR': [-15.7942, -47.8822],# Brazil
    'IO': [-6.3432, 71.8765],  # British Indian Ocean Territory
    'BN': [4.9031, 114.9398],  # Brunei Darussalam
    'BG': [42.6977, 23.3219],  # Bulgaria
    'BF': [12.3714, -1.5197],  # Burkina Faso
    'BI': [-3.3614, 29.3599],  # Burundi
    'CV': [14.9331, -23.5133], # Cabo Verde
    'KH': [11.5564, 104.9282], # Cambodia
    'CM': [3.8480, 11.5021],   # Cameroon
    'CA': [45.4215, -75.6972], # Canada
    'KY': [19.3133, -81.2546], # Cayman Islands
    'CF': [4.3947, 18.5582],   # Central African Republic
    'TD': [12.1048, 15.0444],  # Chad
    'CL': [-33.4489, -70.6693],# Chile
    'CN': [39.9042, 116.4074], # China
    'CX': [-10.4475, 105.6904],# Christmas Island
    'CC': [-12.1642, 96.8708], # Cocos (Keeling) Islands
    'CO': [4.7110, -74.0721],  # Colombia
    'KM': [-11.7022, 43.2551], # Comoros
    'CG': [-4.2634, 15.2429],  # Congo
    'CD': [-4.4419, 15.2663],  # Congo, Democratic Republic of the
    'CK': [-21.2367, -159.7777],# Cook Islands
    'CR': [9.9281, -84.0907],  # Costa Rica
    'CI': [5.3599, -4.0083],   # Côte d'Ivoire
    'HR': [45.8150, 15.9819],  # Croatia
    'CU': [23.1136, -82.3666], # Cuba
    'CW': [12.1696, -68.9900], # Curaçao
    'CY': [35.1856, 33.3823],  # Cyprus
    'CZ': [50.0755, 14.4378],  # Czechia
    'DK': [55.6761, 12.5683],  # Denmark
    'DJ': [11.8251, 42.5903],  # Djibouti
    'DM': [15.3092, -61.3794], # Dominica
    'DO': [18.4861, -69.9312], # Dominican Republic
    'EC': [-0.1807, -78.4678], # Ecuador
    'EG': [30.0444, 31.2357],  # Egypt
    'SV': [13.6929, -89.2182], # El Salvador
    'GQ': [3.7492, 8.7371],    # Equatorial Guinea
    'ER': [15.3229, 38.9251],  # Eritrea
    'EE': [59.4370, 24.7536],  # Estonia
    'SZ': [-26.5225, 31.4659], # Eswatini
    'ET': [9.1450, 40.4897],   # Ethiopia
    'FK': [-51.7963, -59.5236],# Falkland Islands (Malvinas)
    'FO': [62.0079, -6.7900],  # Faroe Islands
    'FJ': [-18.1248, 178.4501],# Fiji
    'FI': [60.1699, 24.9384],  # Finland
    'FR': [48.8566, 2.3522],   # France
    'GF': [4.9224, -52.3269],  # French Guiana
    'PF': [-17.6797, -149.4068],# French Polynesia
    'TF': [-49.2804, 69.3486], # French Southern Territories
    'GA': [0.4162, 9.4673],    # Gabon
    'GM': [13.4549, -16.5790], # Gambia
    'GE': [41.7151, 44.8271],  # Georgia
    'DE': [52.5200, 13.4050],  # Germany
    'GH': [5.6037, -0.1870],   # Ghana
    'GI': [36.1408, -5.3536],  # Gibraltar
    'GR': [37.9838, 23.7275],  # Greece
    'GL': [64.1814, -51.6941], # Greenland
    'GD': [12.1165, -61.6790], # Grenada
    'GP': [16.2650, -61.5510], # Guadeloupe
    'GU': [13.4443, 144.7937], # Guam
    'GT': [14.6349, -90.5069], # Guatemala
    'GG': [49.4657, -2.5853],  # Guernsey
    'GN': [9.6412, -13.5784],  # Guinea
    'GW': [11.8037, -15.1804], # Guinea-Bissau
    'GY': [6.8046, -58.1553],  # Guyana
    'HT': [18.5944, -72.3074], # Haiti
    'HM': [-53.0818, 73.5042], # Heard Island and McDonald Islands
    'VA': [41.9029, 12.4534],  # Holy See (Vatican City State)
    'HN': [14.0723, -87.1921], # Honduras
    'HK': [22.3193, 114.1694], # Hong Kong
    'HU': [47.4979, 19.0402],  # Hungary
    'IS': [64.1466, -21.9426], # Iceland
    'IN': [28.6139, 77.2090],  # India
    'ID': [-6.2088, 106.8456], # Indonesia
    'IR': [35.6892, 51.3890],  # Iran, Islamic Republic of
    'IQ': [33.3128, 44.3615],  # Iraq
    'IE': [53.3498, -6.2603],  # Ireland
    'IM': [54.2361, -4.5481],  # Isle of Man
    'IL': [31.7683, 35.2137],  # Israel
    'IT': [41.9028, 12.4964],  # Italy
    'JM': [18.0179, -76.8099], # Jamaica
    'JP': [35.6895, 139.6917], # Japan
    'JE': [49.2144, -2.1312],  # Jersey
    'JO': [31.9566, 35.9457],  # Jordan
    'KZ': [51.1694, 71.4491],  # Kazakhstan
    'KE': [-1.2864, 36.8172],  # Kenya
    'KI': [1.8439, -157.4718], # Kiribati
    'KP': [39.0392, 125.7625], # Korea, Democratic People's Republic of
    'KR': [37.5665, 126.9780], # Korea, Republic of
    'KW': [29.3759, 47.9774],  # Kuwait
    'KG': [42.8746, 74.5698],  # Kyrgyzstan
    'LA': [17.9667, 102.6000], # Lao People's Democratic Republic
    'LV': [56.9496, 24.1052],  # Latvia
    'LB': [33.8938, 35.5018],  # Lebanon
    'LS': [-29.3639, 27.5079], # Lesotho
    'LR': [6.3165, -10.8074],  # Liberia
    'LY': [32.8872, 13.1913],  # Libya
    'LI': [47.1410, 9.5209],   # Liechtenstein
    'LT': [54.6872, 25.2797],  # Lithuania
    'LU': [49.6116, 6.1319],   # Luxembourg
    'MO': [22.1987, 113.5439], # Macao
    'MG': [-18.8792, 47.5079], # Madagascar
    'MW': [-13.9626, 33.7741], # Malawi
    'MY': [3.1390, 101.6869],  # Malaysia
    'MV': [4.1755, 73.5093],   # Maldives
    'ML': [12.6392, -8.0029],  # Mali
    'MT': [35.8997, 14.5147],  # Malta
    'MH': [7.1315, 171.1845],  # Marshall Islands
    'MQ': [14.6415, -61.0242], # Martinique
    'MR': [18.0735, -15.9582], # Mauritania
    'MU': [-20.3484, 57.5522], # Mauritius
    'YT': [-12.8275, 45.1662], # Mayotte
    'MX': [19.4326, -99.1332], # Mexico
    'FM': [7.4256, 150.5508],  # Micronesia, Federated States of
    'MD': [47.0105, 28.8638],  # Moldova, Republic of
    'MC': [43.7374, 7.4246],   # Monaco
    'MN': [47.8864, 106.9057], # Mongolia
    'ME': [42.4304, 19.2594],  # Montenegro
    'MS': [16.7425, -62.1874], # Montserrat
    'MA': [33.9716, -6.8498],  # Morocco
    'MZ': [-25.9692, 32.5732], # Mozambique
    'MM': [16.8661, 96.1951],  # Myanmar
    'NA': [-22.5609, 17.0658], # Namibia
    'NR': [-0.5228, 166.9315], # Nauru
    'NP': [27.7172, 85.3240],  # Nepal
    'NL': [52.3676, 4.9041],   # Netherlands
    'NC': [-22.2558, 166.4505],# New Caledonia
    'NZ': [-41.2865, 174.7762],# New Zealand
    'NI': [12.1150, -86.2362], # Nicaragua
    'NE': [13.5127, 2.1125],   # Niger
    'NG': [9.0820, 8.6753],    # Nigeria
    'NU': [-19.0544, -169.8672],# Niue
    'NF': [-29.0408, 167.9547],# Norfolk Island
    'MK': [41.6086, 21.7453],  # North Macedonia
    'MP': [15.1852, 145.7455], # Northern Mariana Islands
    'NO': [59.9139, 10.7522],  # Norway
    'OM': [23.5880, 58.3829],  # Oman
    'PK': [33.6844, 73.0479],  # Pakistan
    'PW': [7.5140, 134.5825],  # Palau
    'PS': [31.9522, 35.2332],  # Palestine, State of
    'PA': [8.9824, -79.5199],  # Panama
    'PG': [-9.4438, 147.1803], # Papua New Guinea
    'PY': [-25.2637, -57.5759],# Paraguay
    'PE': [-12.0464, -77.0428],# Peru
    'PH': [14.5995, 120.9842], # Philippines
    'PN': [-25.0663, -130.1005],# Pitcairn
    'PL': [52.2297, 21.0122],  # Poland
    'PT': [38.7223, -9.1393],  # Portugal
    'PR': [18.2208, -66.5901], # Puerto Rico
    'QA': [25.2760, 51.5200],  # Qatar
    'RE': [-21.1151, 55.5364], # Réunion
    'RO': [44.4268, 26.1025],  # Romania
    'RU': [55.7558, 37.6176],  # Russian Federation
    'RW': [-1.9579, 30.1127],  # Rwanda
    'BL': [17.8962, -62.8498], # Saint Barthélemy
    'SH': [-15.9650, -5.7089], # Saint Helena, Ascension and Tristan da Cunha
    'KN': [17.3578, -62.7830], # Saint Kitts and Nevis
    'LC': [13.9094, -60.9789], # Saint Lucia
    'MF': [18.0753, -63.0600], # Saint Martin (French part)
    'PM': [46.9419, -56.2711], # Saint Pierre and Miquelon
    'VC': [13.1600, -61.2248],  # Saint Vincent and the Grenadines
    'WS': [-13.7590, -172.1046],# Samoa
    'SM': [43.9424, 12.4578],   # San Marino
    'ST': [0.1864, 6.6131],     # Sao Tome and Principe
    'SA': [24.7136, 46.6753],   # Saudi Arabia
    'SN': [14.7167, -17.4677],  # Senegal
    'RS': [44.7866, 20.4489],   # Serbia
    'SC': [-4.6796, 55.4920],   # Seychelles
    'SL': [8.4657, -13.2317],   # Sierra Leone
    'SG': [1.3521, 103.8198],   # Singapore
    'SX': [18.0425, -63.0548],  # Sint Maarten (Dutch part)
    'SK': [48.1486, 17.1077],   # Slovakia
    'SI': [46.0569, 14.5058],   # Slovenia
    'SB': [-9.6457, 160.1562],  # Solomon Islands
    'SO': [2.0469, 45.3182],    # Somalia
    'ZA': [-25.7479, 28.2293],  # South Africa
    'GS': [-54.4296, -36.5879], # South Georgia and the South Sandwich Islands
    'SS': [4.8517, 31.5825],    # South Sudan
    'ES': [40.4168, -3.7038],   # Spain
    'LK': [6.9271, 79.8612],    # Sri Lanka
    'SD': [15.5007, 32.5599],   # Sudan
    'SR': [5.8520, -55.2038],   # Suriname
    'SJ': [78.2232, 15.6469],   # Svalbard and Jan Mayen
    'SE': [59.3293, 18.0686],   # Sweden
    'CH': [46.9480, 7.4474],    # Switzerland
    'SY': [33.5138, 36.2765],   # Syrian Arab Republic
    'TW': [25.0330, 121.5654],  # Taiwan, Province of China
    'TJ': [38.5598, 68.7870],   # Tajikistan
    'TZ': [-6.7924, 39.2083],   # Tanzania, United Republic of
    'TH': [13.7563, 100.5018],  # Thailand
    'TL': [-8.5569, 125.5603],  # Timor-Leste
    'TG': [6.1725, 1.2314],     # Togo
    'TK': [-9.2002, -171.8480], # Tokelau
    'TO': [-21.1789, -175.1982],# Tonga
    'TT': [10.6918, -61.2225],  # Trinidad and Tobago
    'TN': [36.8065, 10.1815],   # Tunisia
    'TR': [39.9334, 32.8597],   # Turkey
    'TM': [37.9509, 58.3802],   # Turkmenistan
    'TC': [21.6940, -71.7979],  # Turks and Caicos Islands
    'TV': [-7.4784, 178.6799],  # Tuvalu
    'UG': [0.3476, 32.5825],    # Uganda
    'UA': [50.4501, 30.5234],   # Ukraine
    'AE': [25.276987, 55.296249],# United Arab Emirates
    'GB': [51.5074, -0.1278],   # United Kingdom of Great Britain and Northern Ireland
    'US': [38.9072, -77.0369],  # United States of America
    'UM': [19.2823, 166.6470],  # United States Minor Outlying Islands
    'UY': [-34.9011, -56.1645], # Uruguay
    'UZ': [41.3111, 69.2401],   # Uzbekistan
    'VU': [-17.7333, 168.3270], # Vanuatu
    'VE': [10.4806, -66.9036],  # Venezuela (Bolivarian Republic of)
    'VN': [21.0285, 105.8542],  # Viet Nam
    'VG': [18.4207, -64.6400],  # Virgin Islands (British)
    'VI': [18.3358, -64.8963],  # Virgin Islands (U.S.)
    'WF': [-13.7688, -177.1561],# Wallis and Futuna
    'EH': [24.2155, -12.8858],  # Western Sahara
    'YE': [15.3694, 44.1910],   # Yemen
    'ZM': [-15.3875, 28.3228],  # Zambia
    'ZW': [-17.8252, 31.0335]   # Zimbabwe
}

maxEpo = {
    'GB': 2082084773, 
    'US': 155839750, 
    'JP': 199080595, 
    'DE': 1940729845, 
    'AU': 1788687224, 
    'BR': 1814228073, 
    'CN': 2231391752, 
    'FR': 2091919710, 
    'IN': 1955299841,
    'RU': 2040439493,
    'ZA': 2040439493
}


